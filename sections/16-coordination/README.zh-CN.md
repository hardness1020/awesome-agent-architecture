...
有两份公开的结果讲出了做错的代价。Tran 和 Kiela 给单个 agent 和一个团队同样的 thinking token 预算，在他们测的那些任务上，单个 agent 跟得上。
...
- **只读的内置内容：**放 skill、prompt 和工具定义（第 7 章与第 2 章）。它们在整个运行期间都不会变，所以每个 agent 看到的都一样。
...
2. 已经确认过的事实和要遵守的约束，这样接收的人不会再查一次，也不会踩过去。
