...
底下講的做法，這一章的程式都沒有實作。它們來自 ai-agent-book 對生產環境 agent 的整理，以及公開的多 agent 研究。
這些都不是下面表格那兩套系統已經確認的行為。
...
Tran 和 Kiela 給單一 agent 和一個團隊同樣的 thinking token 預算。在他們測的那些任務上，單一 agent 跟得上。
Anthropic 說他們的 research 團隊會用掉大約單次對話 turn 十五倍的 token。這麼貴的團隊總得帶回點什麼。

**context 是共用還是隔離。**兩個 agent 要嘛共用一份歷史，要嘛各留各的。
共用的話，下一個 agent 直接接手全部，什麼都不用打包，也不會有事實掉在半路。
代價是同一時間只有一個 agent 在跑，而且一個 window 要裝下整個團隊的歷史。
隔離的話，每個 agent 有自己的 window，需要什麼就得講出來。大家可以同時跑，某個 agent 想歪了也停在自己的 window 裡。
代價是每一次交接都得寫下來。
子任務不多、歷史裝得進一個 window、步驟本來就得照順序跑，那就共用。其他情況就隔離。
這個 repo 走的是隔離：subagent 每次都從空的開始（第 6 章），隊友只讀自己的 inbox。

**三種拓撲。**就算 context 隔離了，還是得決定誰跟誰講話。
對等團隊裡，地位相同的 agent 互相傳訊息。互審和交叉檢查適合這種。
管理者團隊裡，一個 lead 把工作拆開、發出去、再把回來的結果合起來。子代回傳的是摘要，不是自己的歷史。
去中心化團隊裡沒有 lead，每個 agent 自己決定下一棒交給誰。
這一章做的是管理者。lead 幫所有人規劃，所以計畫拆錯了就是錯了，下游沒有 worker 補得回來。
這就是為什麼最強的模型要給 lead，worker 用便宜的就好。

**去中心化的團隊怎麼把工作送到人手上。**MetaGPT 把每則訊息丟進一個 pool。每個角色訂閱自己處理的訊息類型，所以 sender 從來不用指名 receiver。
AutoGen 的 group chat 只留一份對話記錄，由中央的 selector 決定下一個誰講話。selector 要是一直挑同樣兩個 agent，這場對話就 livelock 了。
...
共用工作區放 repo、task 看板和 team memory。每個隊友都會往這裡寫，衝突就是在這裡發生的。
這裡需要 lock，或者一人一個 worktree（第 15 章）。
外部掛載放的是團隊自己沒做出來的東西，例如一份 checkout 或一份資料集。往這裡寫，等於動到了團隊外面的東西。
唯讀的內建內容放 skill、prompt 和工具定義（第 7 章與第 2 章）。它們在整個執行期間都不會變，所以每個 agent 看到的都一樣。
...
第一，任務本身，附上接收的人自己就能檢查的驗收標準。
第二，已經確認過的事實和要遵守的限制，這樣接收的人不會再查一次，也不會踩過去。
第三，檔案、log 和 branch 的路徑。
sender 的原始歷史不放進去。那東西很長、裡面都是走不通的路，還會逼接收的人把 sender 犯過的錯再讀一遍。
共用 context 的交接則整個跳過包裹。一個 agent 把控制權交給另一個，整段歷史跟著過去，什麼都不會漏掉。
