...
底下這個設計出自 ai-agent-book，講的是實際上線的 coding agent 怎麼做。本節的 `src/` 並沒有實作它。
下面表格裡的那幾個系統，也沒有證據說它們就是這樣做的。
...
這個做法成本低，有兩個原因。診斷訊息是包在 tool result 裡一起回去的，不用多跑一輪。
檢查只跑一個檔案，不是整個專案，花的時間跟那次寫入差不多。
上面那條規則一樣成立：寫入被擋下就不會執行，自然也不會有診斷訊息。
