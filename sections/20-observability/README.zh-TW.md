...
下面這些，這一章的 `src/` 都沒有實作。它們出自 ai-agent-book 對正式環境 agent 的描述，還有兩套外部的 tracing 標準。
它們也不是下面表格那幾個系統經過確認的行為。

**用 span，不是扁平事件。**扁平的事件流只說得出「有這麼一次呼叫」，說不出這次呼叫屬於哪一步。
一個使用者請求可能變成很多次模型呼叫、工具呼叫和檢索。有些巢狀在別人裡面，有些同時在跑。
事後只靠時間戳把它們理清楚，其實是在猜。

一趟 run 是一條 trace。裡面每一件工作是一個 span。每個 span 記下自己什麼時候開始、跑了多久、有沒有失敗，還有它的 parent 是誰。
所以一趟 run 的 span 會長成一棵樹。從樹頂往下讀，你就看得到哪一步失敗、哪一步慢、每一根分支各花了多少錢。

有兩套標準管這件事。OpenTelemetry 定義 span 本身：一個 trace id、一個 parent id、時間、狀態，還有可以自由填的 attribute。
OpenInference 再補上 LLM 這一塊的名稱：prompt、completion、model、token 數、工具呼叫。
照這套名稱把點埋一次就好。之後換 backend 只是改設定，不用重寫。

匯出也得留在熱路徑之外，理由跟 `emit` 一樣。span 先進佇列，由背景 worker 分批送出。
這樣 collector 再慢，run 也一點都不受影響。這一章的 `emit` 就是它的扁平版；在同一批 event 上補一個 trace id 和一個 parent id，樹就有了。

**非線性的成本與每任務上限。**成本不是跟著步數走。每一輪都要把整段對話重送一次。
第二輪回傳的工具結果，第三、第四、第五輪還要再付一次錢。
context 裡多出來的任何東西，後面每一輪都得再付一遍，所以總額爬升的速度比輪數快。

prompt caching（第 10 章）和 compaction（第 8 章）各砍掉帳單的一塊，但兩邊省下來的不能相加。
compaction 拿掉的，正好是 caching 本來就會打折的那些 token。

一個 session 總額把這些全蓋住了。所以成本要算到任務層級，而且每個任務都給一個上限。
上限攔下一趟 run，跟步數上限攔下停不下來的 loop 是同一招（第 1 章）。
這件事只有書講，書裡也沒有引外部來源，所以這套成本模型就當作者自己的現場經驗看。

**trace 回流成 eval 集。**兩條 pipeline 只在一個方向上交會。正式環境的 trace 變成 eval task。

先挑值得留的那幾趟：報錯的、使用者重試或出手糾正的、花費遠高於其他的。一趟順順利利的 run 給不了新東西。
挑完先脫敏。擋住程式碼和路徑不進 backend 的那份白名單，同樣擋住它們進 task 檔。
然後把每一趟拼回一個 task。一條 trace 裡有起始狀態和每一次工具呼叫，所以它給得出這個 task 的起點，也給得出這趟 run 本來該走到的結果。
