...
这一章负责的是记录：每趟 run 一棵 trace、花掉的钱算得出是哪个任务花的、event 干净到可以存起来也可以拿出去给别人看。

一次改动让质量变好还是变差，那是另一件事，现在有自己的一章（第 23 章）。它吃的就是这一章记下来的东西。
...
evaluation 离线跑，用的是它自己的 task 集（第 23 章）。那些 task 就是从这一章来的。
...
- 每个 event 带一个 parent 链接，扁平的事件流就变成每趟 run 一棵树，跑完之后才读得懂。
- 成本按模型累加成一个 USD 总额，也按任务各自累加，这样单独一趟失控的 run 才不会被平均掉。
- 失败和特别贵的 run，trace 脱敏之后变成 eval task，离线套件才跟得上正式环境真正会遇到的东西。
...
扁平的事件流只说得出「有这么一次调用」，说不出这次调用属于哪一步。一个请求会散成好几次模型调用、工具调用和检索，
有的嵌套、有的并行，事后想靠时间戳拼回去，只能靠猜。

一趟 run 是一条 trace，里面每一件工作是一个 span。每个 span 带开始时间、耗时、状态，还有它 parent 的 id，所以一趟 run 的 span 会长成一棵树。
从树顶往下读，就知道哪一步失败、哪一步慢、每一棵子树各花了多少钱。

撑起这件事的是两套惯例。OpenTelemetry 定义 span 本身：trace id、parent 链接、时间、状态、attribute。
OpenInference 再往上补 LLM 专用的 attribute 名称：prompt、completion、model、token 数、工具调用。
照这套名称埋一次点，换 backend 就只是部署上的选择，不用重写。

导出跟 `emit` 一样不能落在热路径上。span 先排队，由后台 worker 分批送出，所以 collector 慢下来，run 一点都不受影响。
这一章的 `emit` 就是这一切的扁平版；在同一批 event 上补一个 trace id 和一个 parent span id，升级就完成了。
...
- 这个总额是最粗的那个有用数字。它说得出这个 session 花了多少，说不出是哪个任务花的。

agent 的成本不会跟着步数线性长。每一轮都要把整段对话重发一次，所以第二轮返回的工具结果，到第三、第四、第五轮还要再被计费一次。
context 每长出一段，后面每一轮都得再付一遍，所以总花费爬升的速度比较接近轮数的平方，不是轮数本身。
prompt caching（第 10 章）和 compaction（第 8 章）各砍掉其中一块，但两边省下来的不能相加：compaction 拿掉的，正好是 caching 本来就会打折的那些 token。

所以成本要算到任务层级，不能只算 session，而且要按任务设上限。碰到上限就把这趟 run 停掉，跟步数上限拦下停不下来的 loop 是同一招（第 1 章）。
书里讲这套成本模型时没有引外部来源，所以把这个形状当成作者自己的现场经验，不是量出来的曲线。

这一章源码里的 `run_eval` 是最小规模的 eval：把一组固定的 task 集重播到候选 build 上，数过了几题，返回一个比率。
第 23 章在同一个入口下面补上环境、模拟用户和重复运行，也讲清楚为什么比率小幅下滑通常是噪声，不是 regression。

### trace 回流成 eval 集

两条 pipeline 只在一个方向上交会：正式环境的 trace 变成 eval task。

- **筛。**留下值得学的那几趟：报错的、用户重试或出手纠正的、花费远高于中位数的。一趟顺顺利利的 run，套件从里面学不到新东西。
- **脱敏。**挡住代码和路径不进 backend 的那份白名单，同样挡住它们进 task 文件。task 集里带着客户的路径，就没办法拿给需要跑它的人。
- **重播。**trace 里有起始状态和每一次工具调用，所以它拼得回一个 task：这个状态、这个请求、本来应该出现的这个结果。

这件事持续做，eval 集才跟得上线上的真实分布，而不是一开始有人凭感觉猜的那个分布。落到这里的东西，由第 23 章来评分。
...
| **Cons** | 只告诉你发生了什么，答案好不好看不出来。event 是扁平的，一趟 run 得自己手动拼回去。 | 正式环境 telemetry 几乎没有，run 进行中没有 event 流可看。 |
...
- **敏感数据泄漏到 log：**代码、文件路径或 prompt 落进一个一般访问的 backend，或落进由 trace 生出来的 task 文件。
  缓解：白名单可记录字段，送出或存档之前 scrub 掉其余。
- **扁平事件流没有 parent 链接：**没人说得出哪次模型调用属于哪一步，一趟失败的 run 只能靠时间戳重新拼。
