...
每个 agent 如何发出 telemetry、追踪花费、测量质量。

| | Claude Code | mini-swe-agent |
| --- | --- | --- |
| **Pros** | 低成本又安全地换来丰富的正式环境可见度。坏掉的 sink 卡不住也弄不垮 loop。 | crash 掉的 run 也留得下可重建的文件。轨迹文件既是审计记录，也是 eval 语料。 |
| **Cons** | 只告诉你发生了什么，答案好不好看不出来。源码没带 eval，抓不到悄悄的质量 regression。 | 正式环境 telemetry 几乎没有，run 进行中没有 event 流可看。 |
...
| **How: evaluation** | 源码中没有；为重建。一般做法：一组保留的 task 集按 build 评分。 | repo 内置 SWE-bench batch runner，每个 instance 一个 Docker 容器；中断的 batch 可以续跑。 |

---

## 哪里会出错

- **telemetry 落在热路径上：**一个会阻塞或抛异常的 logging 调用会卡住 loop（第 1 章）。缓解：调用完不等结果，搭配 pre-sink 队列与每 sink killswitch。
- **敏感数据泄漏到 log：**代码、文件路径或 prompt 落进一个一般访问的 backend。缓解：白名单可记录字段，送出前 scrub 掉其余。
