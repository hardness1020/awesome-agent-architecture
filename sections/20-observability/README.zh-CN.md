...
**用 span，不是扁平事件。**扁平的事件流只说得出「有这么一次调用」，说不出这次调用属于哪一步。
一个用户请求可能变成很多次模型调用、工具调用和检索。有些嵌套在别人里面，有些同时在跑。
事后只靠时间戳把它们理清楚，其实是在猜。

一趟 run 是一条 trace。里面每一件工作是一个 span。每个 span 记下自己什么时候开始、跑了多久、有没有失败，还有它的 parent 是谁。
所以一趟 run 的 span 会长成一棵树。从树顶往下读，你就看得到哪一步失败、哪一步慢、每一根分支各花了多少钱。

有两套标准管这件事。OpenTelemetry 定义 span 本身：一个 trace id、一个 parent id、时间、状态，还有可以自由填的 attribute。
OpenInference 再补上 LLM 这一块的名称：prompt、completion、model、token 数、工具调用。
照这套名称把点埋一次就好。之后换 backend 只是改配置，不用重写。

导出也得留在热路径之外，理由跟 `emit` 一样。span 先进队列，由后台 worker 分批送出。
这样 collector 再慢，run 也一点都不受影响。这一章的 `emit` 就是它的扁平版；在同一批 event 上补一个 trace id 和一个 parent id，树就有了。

**非线性的成本与每任务上限。**成本不是跟着步数走。每一轮都要把整段对话重发一次。
第二轮返回的工具结果，第三、第四、第五轮还要再付一次钱。
context 里多出来的任何东西，后面每一轮都得再付一遍，所以总额爬升的速度比轮数快。

prompt caching（第 10 章）和 compaction（第 8 章）各砍掉账单的一块，但两边省下来的不能相加。
compaction 拿掉的，正好是 caching 本来就会打折的那些 token。

一个 session 总额把这些全盖住了。所以成本要算到任务层级，而且每个任务都给一个上限。
上限拦下一趟 run，跟步数上限拦下停不下来的 loop 是同一招（第 1 章）。
这件事只有书讲，书里也没有引外部来源，所以这套成本模型就当作者自己的现场经验看。

**trace 回流成 eval 集。**两条 pipeline 只在一个方向上交会。正式环境的 trace 变成 eval task。

先挑值得留的那几趟：报错的、用户重试或出手纠正的、花费远高于其他的。一趟顺顺利利的 run 给不了新东西。
挑完先脱敏。挡住代码和路径不进 backend 的那份白名单，同样挡住它们进 task 文件。
然后把每一趟拼回一个 task。一条 trace 里有起始状态和每一次工具调用，所以它给得出这个 task 的起点，也给得出这趟 run 本来该走到的结果。
