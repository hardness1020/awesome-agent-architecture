...
| **Why** | 計畫只留在 prompt 裡會走丟，所以清單存成 session 狀態。 | session log 才是唯一的事實來源，所以計畫狀態也只是一則事件。 |
| **How: plan artifact** | 一份 todo list 加一個 plan 檔。`TodoWrite` 覆寫清單，從不被管制。 | `todo_write` 把整份清單當成一則事件附加上去，projection 再重放出來。 |
