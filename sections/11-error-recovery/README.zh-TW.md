...
API 層是 timeout、rate limit 和過載。tool 層是指令回傳非零，或是 handler 拋出例外。
context 層是 prompt overflow，還有壞掉的訊息歷史。control flow 層是一直重複、卻走不到任何地方的步驟。
...
下面這些設計出自 ai-agent-book 對生產環境 agent 的紀錄。這一節的 `src/` 沒有實作其中任何一項。
下面那張表裡的系統，也沒有被證實是這樣做的。看的時候當成設計，不要當成觀察結果。

**模型呼叫以外的部分。** 上面那個 helper 只顧到 API 這一層。另外三層各自需要自己的檢查。

**沒有進展。** 每次 tool call 都給它一個 fingerprint：名稱加上參數。同一個 fingerprint 一直出現，就是 agent 在重做同一次呼叫。
這時候沒有東西會拋出例外，所以任何重試路徑都不會啟動。步數上限的確會停下來，但那時預算已經花光了。
fingerprint 計數器幾步之內就能停，而且說得出是哪一次呼叫卡住。
每條復原路徑也各自配一個計數器。一直失敗的那條路，就會自己跳閘。

**沒有活性。** connect timeout 只確認 stream 有沒有接上。連上之後就沒聲音的 stream，它看不到。
那就加一個 idle watchdog。時間窗內沒有 token 進來，就取消這次呼叫。
接著 retry helper 會把這次取消當成一般的暫時性失敗來處理。

**歷史壞掉。** 一輪中途 crash，可能留下一個沒有對應 `tool_result` 的 `tool_use` block。
下一次請求會卡在訊息格式，而不是卡在工作本身。所以送出之前，先把成對關係修好。
至於「修」是什麼意思，得看這份 transcript 是拿來做什麼的。
產品用的 harness 會塞一個 placeholder 結果，寫明這次呼叫被中斷，執行就能繼續。
拿來錄訓練資料的 harness 則拒絕修補。編一個結果出來，等於教模型一個根本沒發生過的步驟。

**復原要分級。** 復原不是一個決定。要照呼叫端該看到多少來分級。

1. 安靜重試。呼叫端只看得到最後的結果。
2. 降級後繼續。回傳一份縮水的結果，並說清楚少了什麼。
3. 把失敗浮現出來。列出試過哪些方法，讓模型可以換一條路走。

前兩級產生的錯誤留在 helper 裡面。等復原真的放棄了，才放出去。
太早傳到模型面前的錯誤，會被當成最終結果，模型可能因此重做一件其實已經成功的工作。

復原也可能自己餵自己。錯誤路徑上可能還會觸發 hook、摘要或通知。
那些事情又去呼叫一次模型，然後又失敗一次。所以在錯誤路徑上，把帶副作用的邏輯關掉。
另外帶一個遞迴深度計數器，把漏網的鏈條切斷。
背景呼叫則完全不重試。它們不在關鍵路徑上，重試只會把主 loop 需要的額度花掉。

界限要照實際量到的失敗來訂，不是靠直覺。書裡「壓縮試三次就停」這個界限，就是從生產環境反覆復原失敗的數據裡得出來的。
