...
以下讲的是正式产品级别的 agent 怎么处理 context，取自 ai-agent-book 对这些系统的说明。
这些做法 `src/` 都没有实现，也没有在下面表格那几个系统上被证实。

**冻结的 tool 结果 stub。**用来替换 tool 结果的那段文字，每次都要一模一样。
第一次替换时就把它定下来，之后照抄，连 session 从磁盘还原回来也照抄。
stub 如果重新算过，带上新的时间戳或新的路径，前缀就变了，它后面的 cache 也没了。

**压缩和 cache 的拉扯。**动过历史，改动点之后的 cache 就全部失效。
每个 turn 都修一点，下一次调用就每个 turn 都要重读整段前缀。
累积到 token 阈值再一次修完，这笔账只付一次。压缩要跑在两次 API 调用之间，不能跑在一次调用里面。

**API 层的 context editing。**这类 pass，Claude API 可以在服务器端直接做掉。
context editing 会把比较旧的 tool 结果从前缀里拿掉，harness 这边一行代码都不用写。
但它一样要重建一次 cache，所以位置比较适合摆在靠近 overflow 那一端，不是每个 turn 都用。

**针对任务写摘要。**摘要要为当前任务而写，不是把整个 session 从头回顾一遍。
它要回答的问题只有一个：下一次调用还需要什么。有四样东西要留，按这个顺序：
...
最先丢掉的是原始 tool 输出。大的结果 budget pass 早就写进磁盘了，真的需要再读回来就好。
