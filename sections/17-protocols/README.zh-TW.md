...
協調（第 16 章）給了 agent 一個管道，但管道只搬運文字。單靠文字沒有契約。

protocol 是疊在管道之上的約定規則：一則請求與其回覆長什麼樣子，以及一則回覆如何對應到它所回答的請求。

有兩種交換最需要它。lead 在隊友編輯到一半時把它殺掉，會留下一個寫到一半的檔案和一筆開著的 task 記錄。

隊友在沒有詢問的情況下跑一個有風險的重構，是先行動、後回報。

兩者都想要同一種形狀：一方請求，另一方回覆，一個 id 把它們綁在一起。
...
沒有這一層，協調就是沒有結構的閒聊。沒有東西被設閘門，沒有東西乾淨地停止，而回覆也無法對應到它所回答的內容。
...
- **A small state machine。** 一筆請求從 `pending` 走到 `approved` 或 `rejected`。針對一個已解析 id 的回覆會被忽略，所以重複是無害的。

shutdown 與 plan 這兩個流程是彼此的鏡像。shutdown 是 lead 請求、隊友確認。plan approval 是隊友請求、lead 確認。

審核結果也可以攜帶工作所在的權限模式，這樣裁決與模式就一起傳遞（第 3 章）。
...
- `StopTeammate` 送出一筆 `shutdown_request`；隊友的 `run_teammate` 確認它並返回。這次停止是一場 handshake，不是一次 kill。
- lead 把回傳的 `shutdown_approved` 解析成 `approved`。主 process 只是等待。
- plan-approval 流程是對稱的反向（先 `ExitPlanMode` 再 `ApprovePlan`），由相同的工具驅動，並在 test.py 裡驗證。
- loop 沒有改變。protocol 透過在管道上塑形請求並解析回覆來包住 turn。

---

## 各系統做法

一種設計如何塑形請求、為計畫設閘門，並乾淨地停止 agent。

| System                | Message shape                                  | Plan approval                     | Shutdown                     |
| --------------------- | ---------------------------------------------- | --------------------------------- | ---------------------------- |
| **Claude Code** | 在 `type` 上的 typed union，帶一個 `request_id`。 | 隊友請求，lead 審核。 | 請求、確認，然後 kill。 |

### Claude Code

- `SendMessageTool` 攜帶一個以 `type` 區分的 `StructuredMessage` union。
- `request_id` 把一則回覆對應到它的請求。
- 訊息 schema 位於 `utils/teammateMailbox.ts`。
- 一個 `plan_mode_required` 的隊友呼叫 `ExitPlanModeV2Tool` 時，會把一則 `plan_approval_request` 寫到 `team-lead` mailbox 並設定 `awaitingPlanApproval`。
- lead 用 `plan_approval_response` 回覆：`approved`、可選的 `feedback`、可選的 `permissionMode`。
- `tasks/stopTask.ts` 要求 `status === 'running'`，呼叫 `taskImpl.kill`，把 task 標記為 `notified`，並發出一個 terminated 事件。
- 優雅路徑在 `gracefulShutdown` 之前先跑 `shutdown_request`，然後 `shutdown_approved` 或 `shutdown_rejected`。

> **取捨：** 具型別的 handshake 讓每一次停止都經過確認、每一個有風險的計畫都設了閘門。
> 代價是往返次數與 protocol 狀態。
> fire and forget 的 kill 比較快，但會遺失進行中的工作並洩漏 task 記錄。
...
loop 與 subagent 路徑不變。protocol 透過在管道上塑形請求並解析回覆來包住 turn。
...
- Claude Code protocol 形狀：`tools/SendMessageTool/SendMessageTool.ts`、`utils/teammateMailbox.ts`。
- Claude Code plan 與 stop：`tools/ExitPlanModeTool/ExitPlanModeV2Tool.ts`、`tasks/stopTask.ts`、`coordinator/coordinatorMode.ts`。
- learn-claude-code · s16_team_protocols：章節框架。
