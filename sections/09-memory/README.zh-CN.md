...
`src/` 只用一个扁平的目录，靠 glob 扫过去，所以每个文件都自动进到索引里。存储区一大就没这种好事了。
大一点的做法是自己维护一份索引文件，并且让记忆文件彼此链接，recall 就能从已经加载的文件往外跟着链接走。
OpenViking 的知识库就是这样做的：文件带 URI、分层摘要、wiki 式的交叉链接。这些 `src/` 都没有实现。
