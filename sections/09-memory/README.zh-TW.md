...
所以問題就變成：一個檔案要怎麼被找到。記憶檔案要進到某一輪只有兩條路，索引上有它那一行，或是已經載入的檔案連過去。
沒有東西會去掃內文，所以兩條路都沒有的檔案，寫了也讀不到。
...
- **記憶檔案沒人連得到：**索引沒列、也沒有別的檔案連過去的檔案，recall 永遠碰不到它。它佔了硬碟，卻答不出任何東西。
  寫檔案的那一步就順手把索引那行寫進去，相關的檔案也要互相連結，就像 OpenViking 這類檔案系統知識庫的做法。
