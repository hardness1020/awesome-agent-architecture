...
`src/` 只用一個扁平的目錄，靠 glob 掃過去，所以每個檔案都自動進到索引裡。儲存區一大就沒這種好事了。
大一點的做法是自己維護一份索引檔案，並且讓記憶檔案彼此連結，recall 就能從已經載入的檔案往外跟著連結走。
OpenViking 的知識庫就是這樣做的：檔案帶 URI、分層摘要、wiki 式的交叉連結。這些 `src/` 都沒有實作。
