"""Agent loop (sections 1 to 7). Unchanged from section 5.

A skill (skills.py) loads through the tool runtime like any other tool: its
body returns as a tool result and enters messages[], so section 7 adds a tool,
not loop code. The loop body stays the four numbered steps.
"""
