...
**分类。**扁平的 registry 看不出一份目录长什么样子。工具依调用往哪里去、又碰到什么，可以分成五类。
感知读外面的世界，执行改动它，协作找上另一个 agent，事件触发让外界把 agent 叫醒，跟用户沟通则是找上人。
第 6、12、16 章做协作，第 13、14 章做事件触发，第 19 章做跟用户沟通的通道。这一章做的是这五类共用的那一层。

**粒度。**两个工具做的事情差不多、吃的输入也差不多，就合并起来。一个 `read_document` 带一个类型参数，比每种文件格式各配一个读取工具好用。
参数开始不一样了就拆开。一份 schema 如果把互不相干的字段全凑在一起，它讲不出哪些字段该填，模型就会填错。

**description 怎么写。**`description` 不是文档。模型在挑工具之前，能看的只有它。写得好的会讲清楚什么时候该用、什么时候不该用，
给出真实的参数值、返回长什么样，还有调用一次要花多少代价。与其再多写一段说明，不如附几个实际的调用例子。
书里说加了例子效果提升很多，但那个数字没有出处，所以只取方向，别当成量级。

**参数保真。**harness 要把 input 原封不动交给 handler。假设它把引号规范化了、把空白修掉了，或多塞了一个模型根本没写的参数，
这次调用就会坏在模型看不到的地方。它送出去的 input 是对的，结果却说编辑没对上，transcript 里也找不到任何解释。
输入不合法就拒绝，并讲明理由。不要自己动手改。

**检查用参数。**有些参数存在就是为了被忽略。像 `expected_price` 这种参数，逼模型在调用真的跑起来之前，先把它以为的数字写下来。
handler 不会照那个数字做事。它读的是存起来的值，决定也照那个值下，两边对不上就记一笔。
这样最后一道检查站的就是模型伪造不了的数据。τ-bench 的评分也是这样：看数据库最后的状态，不看 agent 自己说它做了什么。

**感知工具的接口。**感知工具找到的东西，通常比 context 装得下的还多。有三条规则能让结果诚实。
搜索一次只回一页候选，外加一个 cursor。读取吃 offset 和 limit，模型才走得完一个长文件。截断要标在结果里。
默默截掉比直接报错更糟，因为模型会把一份残缺的文件当成完整的在读。
...
Claude Code 不建索引。它一步一步搜：先 glob、再 grep、然后读文件，模型在每次调用之间把查询收窄。
书里描述 Cursor 走的是另一条路，花成本建索引，好让一句白话查询也能找到没有指名任何标识符的代码。
...
Claude Code 用的是精确的旧字符串替换，而且逼模型先读过文件，所以字符串一过期就直接报错，不会改错行。
书里描述 Cursor 改送一份粗略的骨架，再由第二个训练过的 model 依它把文件重写一遍，并说这条路比较快。

**提前启动与失败范围。**能重叠的不只是批次。一个调用只要自己的参数解析完就可以先跑，这时模型还在写同一批的其他调用。
这样就把这个调用的延迟藏进生成里了。它需要一条失败规则：出错只停掉依赖它的那些调用。
同一批里互不相干的调用照跑，外面那个 turn 也照跑。
...
- **共用一个常驻 session：**书里把共用一个终端当成默认。`cd`、export 出去的变量、启用中的虚拟环境，全都留得住。
  要并行做事时，另外开几个隔离的 shell 就好。模型少打很多重复的设置命令，harness 则多了一份 session 状态要跟踪和重置。

**大目录的发现。**目录一大就不可能整份送出去，所以 registry 先送名称，等有人要了才加载完整的 schema。
提出需求的可以是模型自己，用白话讲就行。MCP-Zero 让 agent 说出自己缺哪一种能力，先配对到 server，再配对到那台 server 上的工具，最后只注入配到的那份 schema。
模型从头到尾都不必知道那个工具存在，这是关键字搜索做不到的。

**对 cache 友好的加载。**schema 放在哪里有差。附在 context 最后面一次，然后就别再动它。
去改 prompt 前面那块工具定义，会让缓存住的前缀连同后面每一个 token 一起失效（第 10 章）。
