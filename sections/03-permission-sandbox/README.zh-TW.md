...
下面三個設計出自 ai-agent-book 對 production coding agent 的描述。這一章的可執行程式都沒有做這些事，
下面表格裡的系統也沒有一個確認有這些行為。把它們當成設計來讀，不是當成觀察到的行為。

**Reading the command, not matching it.** `decide()` 是照工具名稱管制的。這對 shell 工具不夠用。
一個工具名稱底下涵蓋了機器上所有的程式，所以真正要判斷的是那串指令。最常見的第一招是拉一份字串 deny 清單，而這招會失敗。
`rm -rf /` 很好抓。`find . -exec rm {} \;` 把刪除塞在 flag 裡面。`$(echo rm) -rf /` 是執行的時候才把 `rm` 這個字拼出來。
`curl -o /etc/crontab` 則是寫了一個檔案，卻沒用到任何一個寫入指令。

解析器抓得到這幾種。它先把指令拆成程式和參數，也知道哪些 flag 後面要吃掉一個值，所以分得出參數和 flag。
接著它問每個解出來的程式會做什麼。`-exec` 自己帶一條指令，那條指令也要一起檢查。
`-o` 指的是要寫入的檔案，那個路徑就當成一次寫入來檢查。檢查看的是指令會做什麼，不是它怎麼拼。

同樣的想法也適用在結果上。破壞性的捷徑一樣留得下正確的終態：把資料表砍掉再建一次可以，把目錄刪掉再 clone 一份也可以。
結果檢查（第 21 章）兩種都會放行，因為它只看終態。所以 gate 也要限制 agent 是怎麼走到那裡的。
有些動作就算做出來的結果過得了關，照樣要擋。

**What the sandbox actually limits.** gate 決定什麼能跑，沙箱決定決定錯了要付多少代價。有三個限制做掉大部分的工。
...
**Keeping the ask path fast.** 一次 `ask` 本來就要花掉使用者一個回合。如果檢查本身又慢，使用者就要等兩次：
先等 harness 算出它到底要不要問，再等提示框。推測式檢查把第一段等待拿掉。

做法是這樣。harness 把 permission 檢查丟到背景跑，畫面上馬上先顯示一行進度。那行進度不會改動系統上的任何東西。
如果那行還在顯示的時候檢查就回了 `allow`，工具直接執行，使用者根本不會看到提示框。
如果檢查還沒定案，那行進度就換成確認提示。

沒有任何不安全的東西會提早跑，因為提早跑的只有檢查。工具本身還是要等檢查的答案。
