...
下面三个设计出自 ai-agent-book 对 production coding agent 的描述。这一章的可执行程序都没有做这些事，
下面表格里的系统也没有一个确认有这些行为。把它们当成设计来读，不是当成观察到的行为。

**Reading the command, not matching it.** `decide()` 是照工具名称管制的。这对 shell 工具不够用。
一个工具名称底下涵盖了机器上所有的程序，所以真正要判断的是那串命令。最常见的第一招是拉一份字符串 deny 清单，而这招会失败。
`rm -rf /` 很好抓。`find . -exec rm {} \;` 把删除塞在 flag 里面。`$(echo rm) -rf /` 是执行的时候才把 `rm` 这个字拼出来。
`curl -o /etc/crontab` 则是写了一个文件，却没用到任何一个写入命令。

解析器抓得到这几种。它先把命令拆成程序和参数，也知道哪些 flag 后面要吃掉一个值，所以分得出参数和 flag。
接着它问每个解出来的程序会做什么。`-exec` 自己带一条命令，那条命令也要一起检查。
`-o` 指的是要写入的文件，那个路径就当成一次写入来检查。检查看的是命令会做什么，不是它怎么拼。

同样的想法也适用在结果上。破坏性的捷径一样留得下正确的终态：把数据表砍掉再建一次可以，把目录删掉再 clone 一份也可以。
结果检查（第 21 章）两种都会放行，因为它只看终态。所以 gate 也要限制 agent 是怎么走到那里的。
有些动作就算做出来的结果过得了关，照样要挡。

**What the sandbox actually limits.** gate 决定什么能跑，沙箱决定决定错了要付多少代价。有三个限制做掉大部分的活。
...
**Keeping the ask path fast.** 一次 `ask` 本来就要花掉用户一个回合。如果检查本身又慢，用户就要等两次：
先等 harness 算出它到底要不要问，再等提示框。推测式检查把第一段等待拿掉。

做法是这样。harness 把 permission 检查丢到后台跑，界面上马上先显示一行进度。那行进度不会改动系统上的任何东西。
如果那行还在显示的时候检查就回了 `allow`，工具直接执行，用户根本不会看到提示框。
如果检查还没定案，那行进度就换成确认提示。

没有任何不安全的东西会提前跑，因为提前跑的只有检查。工具本身还是要等检查的答案。
