...
只有一個過或不過，訊號太薄。把驗證拆成三層，這趟 trajectory 才有東西可學，而且每一層都要自己帶證據：

- **結果（outcome）：**最後的狀態有沒有符合任務的要求。證據是這趟執行留下來的狀態，能用程式檢查的就用程式檢查（第 23 章）。
- **過程（process）：**該遵守的規則有沒有遵守：能用哪些 tool、順序對不對、該確認的有沒有跳過。證據是 trace 裡的 tool 呼叫。
- **品質（quality）：**面對程式檢查不出來的 rubric，輸出夠不夠好。證據是評過的每一條 rubric，還要指名是哪一條沒過。

結果過了但過程沒過，代表這趟只是運氣好。過程過了但結果沒過，代表規則本身有問題。要修哪裡，看的就是這個差別。
...
改進 loop 難的不是找出教訓，而是決定這個教訓要放進哪裡。能放的地方有四種：

- **知識文件：**某趟執行發現的一個事實。寫進去便宜，刪掉也便宜，任務需要的時候再讀回來（第 9 章）。
- **Prompt 或 skill：**一種希望每次都重複的行為。代價是只要載入，每個 turn 都得付 context（第 7 章）。
- **程式：**一段每次都跑得一模一樣的流程。推論時完全不花錢，而且測得起來（第 2 章）。
- **模型權重：**最後手段。慢、貴、最難回頭，也不在這個 repo 談的 harness 範圍裡。

分流規則是：挑一個裝得下這個改動、而且最小、最好驗、最好退回去的地方。
一個事實不該塞進 system prompt，一段流程也不該寫成一大段文字說明。

改 prompt 是最容易被濫用的一種，所以要設關卡。diff 得從反覆出現的失敗來寫，不能只憑一次，而且要框住觸發它的那個條件，免得不相干的執行也被它影響。
驗兩次：一次拿改動附近的邊界案例驗，一次拿寫這個 diff 時沒看過的 holdout set 驗。接著先放一部分流量做 canary，回退方案隨時備著。
Karpathy 把這件事叫 system prompt learning：改的是文字，不是權重。ACE 則讓 diff 保持很小，用穩定的 id 去改 context 裡的單項，而不是把整段 prompt 重寫。

「程式」這個去處是前面章節沒做過的。skill 交給 model 的是一份它還得自己讀、自己照做的指示（第 7 章）；
編譯出來的 workflow 交給 harness 的則是一段不用 model 就能跑的程式。從前者變成後者有五步：

1. **捕捉（capture）：**把一趟成功的 trajectory 記下來：呼叫了什麼、順序如何、每一步前後的狀態長什麼樣。
2. **參數化（parameterize）：**把每趟執行都不一樣的部分換成參數，剩下不變的就是那段程式。
3. **在重置環境驗證（validate on reset）：**在乾淨的環境重放一次（第 23 章），每一步都有前置條件、後置條件和最終狀態檢查。
4. **重放（replay）：**下次遇到同類任務就直接跑這段程式。不用呼叫 model，所以又快又便宜，而且每次結果都一樣。
5. **失效（invalidate）：**只要有一項檢查沒過就讓這段程式退場，任務交還給 model，由它去捕捉新的一段。

工具創建走的是同一條生命週期，只是從另一頭開始：發現能力缺口，agent 就去找現成的函式庫、把它包成一個 tool，驗證過了才放進 registry（第 2 章）。
兩件事都是把一次昂貴的探索變成便宜又檢查得動的能力。兩件事也都少不了第五步，因為當初編譯時依賴的環境一定會變。

### 改 harness 本身

會動 harness 程式碼的改進 loop，需要的不是 patch，而是先有一份契約。這份變更契約要寫清楚四件事：
證據（哪些 trace 失敗、失敗得多頻繁）、根本原因、預期效果，以及怎麼退回去。
沒有契約就不准動手。契約同時也是人稽核時看的東西，有沒有它，決定了這是一個能自我修改的 loop，還是一個沒人審得動的 loop。
候選空間同樣要收斂：loop 只能在事先宣告好的程式範圍內提修改，權限、budget 和關卡都放在那個範圍外面（第 3 章）。

搜尋的尺度是一道階梯：prompt 裡的一條規則、context 怎麼組裝、workflow 有哪些步驟、harness 程式碼，最後是優化器本身。
下一階失敗了才往上爬一階，因為每爬一階，搜尋空間都更大，驗證都更弱。
一條 prompt 規則一天就能跑完 A/B 測試；換掉優化器，改的是之後每一個修改的提出方式。

### 線上執行，離線學習

這兩件事要分開。線上的 loop 只負責把任務跑完、把證據記下來，不做提煉、不升級 skill、也不改 prompt。
離線的 loop 才把大量 trajectory 拿來一起讀，診斷反覆出現的失敗，產出候選改動，驗證過了才發布成一個版本。

分開之後，單獨一趟執行就改不動整個 agent。一條走運的路徑不算規律，某個網頁叫 agent 記住的話更不算證據。
要求跨 trajectory 都有支持，再加上一道驗證關卡，這兩種東西就進不了正式發布。拆開之後，該量的東西也變了，要看的是兩個數字，不是一個：

- **更新（updating）：**這個 loop 產出的候選好不好。提了幾個、幾個通過驗證、幾個被退回去。
- **收益（benefit）：**發布出去的改動有沒有用。該生效的執行有沒有生效、載入之後 agent 有沒有真的照做、holdout 上的表現有沒有變好。

沒有這個拆分，一個內容正確但從來沒被載入的 skill，看起來會像一次失敗的更新，loop 對自己的認識就學歪了。
...
- **代理目標偏移（Proxy goal drift）：**開放式的工作裡，rubric 只是真正目標的替身。loop 會去優化這個替身：挑熟悉的做法、把雜訊當成發現、只留下有過的執行。
  分數一路往上，目標卻在偏。緩解：把失敗的結果也留在證據裡、定期換新的 holdout set，並拿真正的目標去人工抽查輸出。
- **每個教訓都變成改 prompt（Every lesson becomes a prompt edit）：**最便宜的去處會把所有東西都吸過去，system prompt 一路長大，長到自己的規則互相打架。
  緩解：按去處分流。事實進記憶、流程寫成程式，prompt 只留給非重複不可的行為。
- **編譯好的 workflow 活得比環境久（A compiled workflow outlives its environment）：**網站或 API 改過之後，重放的程式照跑不誤，寫錯狀態的速度比 model 還快。
  緩解：每次重放都做前置和後置檢查，第一項檢查沒過就讓它失效。
- **線上執行自己升級自己的教訓（The online run promotes its own lessons）：**執行途中就在提煉經驗的 agent，可能把一條走運的路徑升上去，也可能把不可信網頁故意留給它記住的文字升上去。
  緩解：線上的 loop 只記證據，發布前由獨立的離線流程驗證候選。
