...
只有一个过或不过，信号太薄。把验证拆成三层，这趟 trajectory 才有东西可学，而且每一层都要自己带证据：

- **结果（outcome）：**最后的状态有没有符合任务的要求。证据是这趟执行留下来的状态，能用代码检查的就用代码检查（第 23 章）。
- **过程（process）：**该遵守的规则有没有遵守：能用哪些 tool、顺序对不对、该确认的有没有跳过。证据是 trace 里的 tool 调用。
- **质量（quality）：**面对代码检查不出来的 rubric，输出够不够好。证据是评过的每一条 rubric，还要指名是哪一条没过。

结果过了但过程没过，说明这趟只是运气好。过程过了但结果没过，说明规则本身有问题。要修哪里，看的就是这个差别。
...
改进 loop 难的不是找出教训，而是决定这个教训要放进哪里。能放的地方有四种：

- **知识文档：**某趟执行发现的一个事实。写进去便宜，删掉也便宜，任务需要的时候再读回来（第 9 章）。
- **Prompt 或 skill：**一种希望每次都重复的行为。代价是只要加载，每个 turn 都得付 context（第 7 章）。
- **程序：**一段每次都跑得一模一样的流程。推理时完全不花钱，而且测得起来（第 2 章）。
- **模型权重：**最后手段。慢、贵、最难回头，也不在这个 repo 谈的 harness 范围里。

分流规则是：挑一个装得下这个改动、而且最小、最好验、最好退回去的地方。
一个事实不该塞进 system prompt，一段流程也不该写成一大段文字说明。

改 prompt 是最容易被滥用的一种，所以要设关卡。diff 得从反复出现的失败来写，不能只凭一次，而且要框住触发它的那个条件，免得不相干的执行也被它影响。
验两次：一次拿改动附近的边界案例验，一次拿写这个 diff 时没看过的 holdout set 验。接着先放一部分流量做 canary，回退方案随时备着。
Karpathy 把这件事叫 system prompt learning：改的是文字，不是权重。ACE 则让 diff 保持很小，用稳定的 id 去改 context 里的单项，而不是把整段 prompt 重写。

“程序”这个去处是前面章节没做过的。skill 交给 model 的是一份它还得自己读、自己照做的指示（第 7 章）；
编译出来的 workflow 交给 harness 的则是一段不用 model 就能跑的程序。从前者变成后者有五步：

1. **捕捉（capture）：**把一趟成功的 trajectory 记下来：调用了什么、顺序如何、每一步前后的状态长什么样。
2. **参数化（parameterize）：**把每趟执行都不一样的部分换成参数，剩下不变的就是那段程序。
3. **在重置环境验证（validate on reset）：**在干净的环境重放一次（第 23 章），每一步都有前置条件、后置条件和最终状态检查。
4. **重放（replay）：**下次遇到同类任务就直接跑这段程序。不用调用 model，所以又快又便宜，而且每次结果都一样。
5. **失效（invalidate）：**只要有一项检查没过就让这段程序退场，任务交还给 model，由它去捕捉新的一段。

工具创建走的是同一条生命周期，只是从另一头开始：发现能力缺口，agent 就去找现成的库、把它包成一个 tool，验证过了才放进 registry（第 2 章）。
两件事都是把一次昂贵的探索变成便宜又检查得动的能力。两件事也都少不了第五步，因为当初编译时依赖的环境一定会变。

### 改 harness 本身

会动 harness 代码的改进 loop，需要的不是 patch，而是先有一份契约。这份变更契约要写清楚四件事：
证据（哪些 trace 失败、失败得多频繁）、根本原因、预期效果，以及怎么退回去。
没有契约就不准动手。契约同时也是人审计时看的东西，有没有它，决定了这是一个能自我修改的 loop，还是一个没人审得动的 loop。
候选空间同样要收敛：loop 只能在事先声明好的代码范围内提修改，权限、budget 和关卡都放在那个范围外面（第 3 章）。

搜索的尺度是一道阶梯：prompt 里的一条规则、context 怎么组装、workflow 有哪些步骤、harness 代码，最后是优化器本身。
下一阶失败了才往上爬一阶，因为每爬一阶，搜索空间都更大，验证都更弱。
一条 prompt 规则一天就能跑完 A/B 测试；换掉优化器，改的是之后每一个修改的提出方式。

### 线上执行，离线学习

这两件事要分开。线上的 loop 只负责把任务跑完、把证据记下来，不做提炼、不升级 skill、也不改 prompt。
离线的 loop 才把大量 trajectory 拿来一起读，诊断反复出现的失败，产出候选改动，验证过了才发布成一个版本。

分开之后，单独一趟执行就改不动整个 agent。一条走运的路径不算规律，某个网页叫 agent 记住的话更不算证据。
要求跨 trajectory 都有支持，再加上一道验证关卡，这两种东西就进不了正式发布。拆开之后，该量的东西也变了，要看的是两个数字，不是一个：

- **更新（updating）：**这个 loop 产出的候选好不好。提了几个、几个通过验证、几个被退回去。
- **收益（benefit）：**发布出去的改动有没有用。该生效的执行有没有生效、加载之后 agent 有没有真的照做、holdout 上的表现有没有变好。

没有这个拆分，一个内容正确但从来没被加载的 skill，看起来会像一次失败的更新，loop 对自己的认识就学歪了。
...
- **代理目标偏移（Proxy goal drift）：**开放式的工作里，rubric 只是真正目标的替身。loop 会去优化这个替身：挑熟悉的做法、把噪声当成发现、只留下过了的执行。
  分数一路往上，目标却在偏。缓解：把失败的结果也留在证据里、定期换新的 holdout set，并拿真正的目标去人工抽查输出。
- **每个教训都变成改 prompt（Every lesson becomes a prompt edit）：**最便宜的去处会把所有东西都吸过去，system prompt 一路长大，长到自己的规则互相打架。
  缓解：按去处分流。事实进记忆、流程写成程序，prompt 只留给非重复不可的行为。
- **编译好的 workflow 活得比环境久（A compiled workflow outlives its environment）：**网站或 API 改过之后，重放的程序照跑不误，写错状态的速度比 model 还快。
  缓解：每次重放都做前置和后置检查，第一项检查没过就让它失效。
- **线上执行自己升级自己的教训（The online run promotes its own lessons）：**执行途中就在提炼经验的 agent，可能把一条走运的路径升上去，也可能把不可信网页故意留给它记住的文字升上去。
  缓解：线上的 loop 只记证据，发布前由独立的离线流程验证候选。
