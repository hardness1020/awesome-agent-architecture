...
**few-shot 示例。**示例也放在前綴裡，所以規則一樣。
每次請求都去挑最相關的示例，等於每次呼叫都重寫前綴，cache 就用不上了。
一種任務類型挑一組示例，整個 session 都不要再動它。

**agent 狀態列。**有些 harness 會把執行期狀態濃縮成 context 尾端的幾行字：tool 呼叫次數、當前 TODO、經過時間、工作目錄。
要讓這幾行保持最新有兩種做法，兩種都有代價。
每一輪就地換掉這一段，狀態就只有一份是真的，但尾巴被重寫，後面的 cache 也沒了。
...
**指令與資料。**抓回來的網頁、檔案、issue 留言、MCP server 的回應，全都是資料，沒有一句是使用者說的。
這些文字沒有任何標記就送進去，裡面某一句長得像指令的話，就會跟 system prompt 平起平坐。這就是 prompt injection。
所以 prompt 還得講清楚：哪些文字算命令，哪些不算。
...
這些做法都不是邊界。模型還是可能被說服而違規，所以第 3 章的檢查照樣要跑。
