...
以下讲的是正式产品级别的 agent 怎么组 prompt，取自 ai-agent-book 对这些系统的说明。
...
**few-shot 示例。**示例也放在前缀里，所以规则一样。
每次请求都去挑最相关的示例，等于每次调用都重写前缀，cache 就用不上了。
一种任务类型挑一组示例，整个 session 都不要再动它。

**agent 状态栏。**有些 harness 会把运行期状态浓缩成 context 尾端的几行字：tool 调用次数、当前 TODO、经过时间、工作目录。
要让这几行保持最新有两种做法，两种都有代价。
每一轮就地换掉这一段，状态就只有一份是真的，但尾巴被重写，后面的 cache 也没了。
...
**指令与数据。**抓回来的网页、文件、issue 留言、MCP server 的响应，全都是数据，没有一句是用户说的。
这些文字没有任何标记就送进去，里面某一句长得像指令的话，就会跟 system prompt 平起平坐。这就是 prompt injection。
所以 prompt 还得讲清楚：哪些文字算命令，哪些不算。
...
这些做法都不是边界。模型还是可能被说服而违规，所以第 3 章的检查照样要跑。
