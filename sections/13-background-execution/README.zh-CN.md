...
- 真正的完成结果稍后才会以一则独立的 notification 抵达。
- 一整个 subagent 也可以在后台执行。

### New：移出 loop 的启动，以及 notification 排空
...
loop 在一个 turn 开始时排空待处理的完成 notification：
...
| System | 移出 loop 的原语 | Notification | 重新进入 |
| --- | --- | --- | --- |
| **Claude Code** | 后台 shell task 和后台 agent task。 | `<task_notification>`。 | queue 在 turn 之间排空 notification。 |

### Claude Code

- `BashTool` 支持 `run_in_background`。
- `LocalShellTask` 追踪后台 shell 指令。
- `ShellCommand.background(taskId)` 让 subprocess 继续执行并重定向输出。
- `DreamTask` 在后台执行记忆整合。
- `Task.ts` 追踪后台 task 的状态。
- `enqueueTaskNotification` 把完成消息送到共享 queue。
- 这个 queue 有 `now`、`next` 和 `later` 三种优先级。
- `Sleep` 是一种非阻塞的等待，不会占住一个 shell process。

> **取舍：** 后台执行提升吞吐量，也避免空闲的等待。
> 但它也意味着结果可能较晚抵达，而且顺序可能颠倒。
> runtime 需要 task 状态、notification 和清理机制。
...
- [`loop.py`](src/loop.py)：在调用 model 前排空待处理的 notification。
...
- learn-claude-code · s13_background_tasks：章节框架。
