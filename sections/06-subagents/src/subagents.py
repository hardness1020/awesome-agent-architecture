...
exploration never enters the parent's history. The child's own tool calls still
pass the gate (section 3), so isolating context does not isolate authority.
Recursion is fenced by curating the child's tools: the child registry omits the
Agent tool, so a child cannot spawn another. Mirrors Claude Code's AgentTool +
runAgent.ts (fresh initialMessages, extractTextContent of the last message).
"""
