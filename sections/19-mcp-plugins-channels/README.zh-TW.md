...
於是 agent 不需要任何人動 harness，就得到了 Jira 工具或部署工具。少了它，能力就凍結在二進位檔出貨時的樣子。
...
- 探索是每個 server 一次 `tools/list` 呼叫；每個回傳的規格都會變成一個被包裝的 `Tool`。
...
兩個較小的部件把這一章補齊。一個 server 可以把訊息推回來，包裝成一個帶標籤的區塊，折進下一輪：
...
而一個 plugin 的 server 會依優先序與使用者、專案設定分層疊加：
...
- [`mcp.py`](src/mcp.py)：探索與包裝（`connect`、`wrap`、`tool_name`、`normalize`）、plugin 設定合併（`merge_servers`），以及 channel 包裝（`wrap_channel`）。
- [`test.py`](src/test.py)：探索與命名空間、annotation 到權限提示的對應、連同 gate 合併進池、設定優先序，以及 channel 標籤。
